...
public class ValidadorEntrada {
    ...
    public static String validarYNormalizarPalabra(String palabra) {
        if (palabra == null || palabra.trim().isEmpty()) return null;
        palabra = palabra.trim();
        if (!esSoloLetras(palabra)) return null;
        if (!esLongitudValida(palabra)) return null;
        return quitarAcentos(palabra).toUpperCase();
    }
}